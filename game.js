   ...
   checkCollision(player, object) {
    ...
    updateScore() {
        if (this.scoreElement) {
            this.scoreElement.textContent = `⭐️ Очки: ${this.score}`;
        }
    ...
    gameOver() {
        ...
        if (gameOverScreen) gameOverScreen.classList.remove('hiden');
        if (finalScore) finalScore.textContent = `⭐️ Очки: ${this.score}`;
        if (menu) menu.classList.remove('hiden');
        if (gameContainer) {
            gameContainer.classList.remove('playing');
        }

        this.screenShake = 2;
        this.createParticleEffect(this.player.x + this.player.width/2, this.player.y + this.player.y + this.player.height/2, 30, '#FF0000');
